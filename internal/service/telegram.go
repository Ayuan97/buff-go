...
func sendTelegram(info *model.Goods, source int) {
	//当更新时间为0时，不推送 于当前时间相差10分钟时，不推送
	if info.BuffUpdate == 0 || info.SteamUpdate == 0 || time.Now().Unix()-info.BuffUpdate > 600 || time.Now().Unix()-info.SteamUpdate > 600 {
