...
func TestOrderbookBestBidAndEmpty(t *testing.T) {
	...
	data := OrderBookData{
		MaxBuyOrder:      &max,
		Currency:         evidencedCNYCurrency,
		BuyOrders:        10,
		CompactBuyOrders: []int64{27567, 4, 27293, 1},
	}
	...
	emptyBid := OrderBookData{Currency: evidencedCNYCurrency, BuyOrders: 0}
...
func TestOrderbookRejectsUSDWallet(t *testing.T) {
	min := int64(4114)
	data := OrderBookData{MinSellOrder: &min, Currency: 1, SellOrders: 1, CompactSellOrders: []int64{4114, 1}}
...
func TestParseOrderbookUnwrapsQueryActionEnvelope(t *testing.T) {
	...
	if err != nil || !parsed.Success || parsed.Data == nil || parsed.Data.Currency != 23 {
		t.Fatalf("parsed=%+v err=%v", parsed, err)
	}
	if parsed.Data.MaxBuyOrder == nil || *parsed.Data.MaxBuyOrder != 5617 {
		t.Fatalf("max buy=%v", parsed.Data.MaxBuyOrder)
	}
...
func TestSearchEmptyPage(t *testing.T) {
	parsed, err := parseSearch([]byte(`{"success":true,"start":0,"pagesize":10,"total_count":0,"results":[]}`))
