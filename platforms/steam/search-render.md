...
证据：`steam_ratelimit_20260813.log`。登录 search/render **0×200**。

| 阶段 | UTC | 结果 |
|---|---|---|
| 日首次 | 06:20:35 | **429**（距 08-12 11:03 ≈19h） |
...
冷却上界：08-12→08-13 早段 **>19h** 仍 429；同日 07:47:39→08:48:13 **60m34s** 静默后恢复 200。
...
孤立段 287 不是空桶冷启动：前面已有 107×200。不要写成固定 QPS。突发后恢复：**30s 阶梯仍 429，60s 阶梯 200**。
...
- 空桶冷启动背靠背上限（第一次 429 前已消耗 107 次）
- 重置后突发上限（已测再突发 **500×200** + v2f **4862×200 / 36m2s** 均未 429；随后 302 掉会话）
