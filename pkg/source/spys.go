...
// free-proxy get ip from https://spys.one/en/free-proxy-list/
func Spys() []*model.Ip {
	var ips []*model.Ip

	fmt.Println("开始抓取 https://spys.one/en/free-proxy-list/")
	...
	c.OnHTML("body > table:nth-child(3) > tbody > tr:nth-child(4) > td > table > tbody", func(e *colly.HTMLElement) {
		...
		e.ForEach("tr", func(i int, element *colly.HTMLElement) {
			...
			if i > 3 {
				...
				ips = append(ips, &model.Ip{
					...
					Type:    1,
					Source:  "https://spys.one/en/free-proxy-list/",
				})
			}
		})
	...
	PrimitiveUrl := "https://spys.one/en/free-proxy-list/"
