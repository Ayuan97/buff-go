...
常驻实例使用 PostgreSQL 会话锁，并每 2 秒验证一次；认领代次会拒绝接管实例重新认领后的旧结果。当前尚未把实例代次写入每个写事务，因此锁连接断开到 watchdog 停止旧实例、或接管实例重新认领任务之间仍有一个最长约“2 秒 + 锁验证超时”的故障交接窗口；这不是严格的跨进程单写栅栏。
...
结果先比较方向的开关版本，再比较提交写序，不使用采集时间推断提交先后。每个方向只保留最近成功提交的一个采集批次：ask 批次是一页搜索结果，bid 批次是一个商品的订单簿摘要。最新尝试、最近有效价和价格变化分别保存，变价查询窗口为最近 30 天。

页提交同时校验任务仍由发起提交的组合持有，避免实例锁交接后旧执行者写入新执行者的认领结果。
